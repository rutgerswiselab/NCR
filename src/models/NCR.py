...
class NCR(BaseModel):
    ...
    def __init__(self, label_min, label_max, feature_num, user_num, item_num, u_vector_size, i_vector_size,
                 r_weight, ppl_weight, pos_weight, random_seed, model_path, alpha=10):
    ...
    def predict(self, feed_dict):
        """
        Generate prediction only. No loss calculation here.
        ...
        # Random shuffle the order of sequence to satisfy the associativity and commutativity laws
        ...
        if feed_dict['rank'] == 1:
            prediction = F.cosine_similarity(sent_vector, self.true.view([1, -1])) * self.alpha
        else:
    ...
    def forward(self, feed_dict):
        ...
        # recommendation loss
        if feed_dict['rank'] == 1:
            # calculate pair-wise loss. First half batch are positive scores and second half are
            # corresponding negative scores.
